...
First discuss Figure~\ref{fig:fig1a}.
...
Then Figure~\ref{fig:fig1b}, which is the one where the code was found in a
separate directory, with slides for a talk.
...
Then discuss Table~\ref{tab:tab2}. There is one apparent difference.
...
Then discuss Figure~\ref{fig:fig2}.
...
We can plot the detailed differences between the original and
reproduced figure, shown in Figure~\ref{fig:fig2diff}.
...
essnetial. Table~3 in \citet{lamichhane2003} showed just a subset of
these, omitting those indicated with an asterisk.
...
Finally, we investigated gene families, to see whether particular
families appeared to have an unusually high or low proportion of
essential genes, Table~\ref{tab:tab4}.
