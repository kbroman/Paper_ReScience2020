...
prepData <- 
function(ratio=0.8, num=100, tas=taloc, doubles=doubleta)
{
  # double-hits that are in early 80% of either gene
  doubles <- doubles[doubles[,2]/doubles[,3] <= ratio |
                      doubles[,2] <= doubles[,3]-num |
                     doubles[,5]/doubles[,6] <= ratio |
                      doubles[,5] <= doubles[,6]-num, ]
  ...
  tas <- tas[tas[,2]/tas[,3] <= ratio | tas[,2] <= tas[,3] - num, ]
...
mydata <- prepData(0.8, 100)

# number of TA sites for each gene
numTAs <- mydata[,1]+mydata[,3]+ c(0,mydata[-nrow(mydata),3])

# gene classes
geneclasses <- info[match(rownames(mydata),info[,1]),3]
ngeneperfam <- table(factor(geneclasses,levels=1:109))

# Summary of no. TAs and no. hits
c(sum(mydata[,1])+sum(mydata[,3]), sum(mydata[,3]),
  sum(mydata[,2])+sum(mydata[,4]), sum(mydata[,4]), nrow(mydata),
  sum(mydata[,2]>0 | mydata[,4]>0 | c(0,mydata[-nrow(mydata),4])>0))
