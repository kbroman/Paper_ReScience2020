...
mut <- read.csv("../Rawdata/GenomicData-final4.csv")
mut <- mut[,-c(3,5)]
...
#kill <- mut[mindiff>=10,] # 2402:1 and 3851:1
#wh <- (1:nrow(taloc))[taloc[,1]==2402]
#taloc[wh[1],4] <- taloc[wh[1],4] - 1
#wh <- (1:nrow(taloc))[taloc[,1]==3851]
#taloc[wh[length(wh)],4] <- taloc[wh[length(wh)],4] - 1
#mut <- mut[mindiff<10,]
...
prepData <- 
function(ratio=0.8,tas=taloc,doubles=doubleta)
{
